...
async function handlePlaceOrder(e) {
    ...
    const paymentMethod = rawPayment in PAYMENT_METHOD_LABELS
        ? PAYMENT_METHOD_LABELS[rawPayment]
        : rawPayment;
    const orderPayload = {
...
function onOrderPlaced(result, paymentMethod) {
    ...
    cartItems = [];
    const trackedOrderIds = JSON.parse(sessionStorage.getItem("trackedOrderIds") || "[]");
...
function renderCartItems() {
    ...
    if (cartItems.length === 0) {
        ...
        `;
        if (heading)
            heading.textContent = "ORDER ITEMS (0)";
        return;
    }
    if (heading)
        heading.textContent = `ORDER ITEMS (${cartItems.length})`;
...
function attachQuantityHandlers() {
    document.querySelectorAll(".order-items .qty-btn").forEach(btn => {
        btn.addEventListener("click", (e) => {
            ...
            const row = currentTarget?.closest(".item-row");
...
function updateOrderSummary() {
    ...
    if (summaryRows[0])
        summaryRows[0].textContent = `${subtotal.toFixed(3)} OMR`;
    if (summaryRows[1])
        summaryRows[1].textContent = `${DELIVERY_FEE.toFixed(3)} OMR`;
    if (summaryRows[2])
        summaryRows[2].textContent = `${vat.toFixed(3)} OMR`;
    const totalPriceEl = document.querySelector(".summary-row.total-row .total-price");
    if (totalPriceEl)
        totalPriceEl.textContent = `${total.toFixed(3)} OMR`;
    const placeOrderBtn = document.querySelector(".btn-place-order");
    if (placeOrderBtn)
        placeOrderBtn.textContent = `Place Order (${total.toFixed(3)} OMR)`;
}
