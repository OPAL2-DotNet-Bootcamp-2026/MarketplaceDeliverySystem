// Global Window interface augmentation for inline HTML onclick attributes
  ...
  interface Window {
    ...
    addProductToOrder: (productId: number, productName: string) => void;
  }

interface BusinessHeaderDTO {
  businessName: string;
  isOpen: boolean;
...
async function loadBusinessHeader(businessId: string): Promise<void> {
  const container = document.querySelector<HTMLElement>("#business-info-container");
  if (!container) return;

  try {
    const response = await fetch(`${BASE_HEADER_API}/${encodeURIComponent(businessId)}`);
    if (!response.ok) throw new Error("Failed to load business header details.");

    const b: BusinessHeaderDTO = await response.json();
...
async function loadProducts(businessId: string, categoryId: string | null): Promise<void> {
  const listContainer = document.querySelector<HTMLElement>("#products-list-container");
  if (!listContainer) return;

  try {
    let url = `${BASE_PRODUCTS_API}/${encodeURIComponent(businessId)}`;
    if (categoryId) {
    ...
    const badge = document.querySelector<HTMLElement>("#available-items-badge");
...
function buildCategoryPills(): void {
  const pillsContainer = document.querySelector<HTMLElement>("#product-pills-container");
  if (!pillsContainer) return;

  const validCategories = allProducts
  ...
  pillsContainer.querySelectorAll<HTMLAnchorElement>(".subcategory-pill").forEach((pill) => {
...
function renderFilteredProducts(): void {
  const listContainer = document.querySelector<HTMLElement>("#products-list-container");
  const modalsContainer = document.querySelector<HTMLElement>("#product-modals-container");
          ...
          <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-sm-nowrap">
            ...
            <div class="d-flex align-items-center gap-2 ms-auto ms-sm-0">
              ...
              <button type="button" 
                      ...
                      onclick="addProductToOrder(${product.productId}, '${product.productName.replace(/'/g, "\\'")}')">
                Add to Order
              </button>
            </div>
          </div>
            ...
            <div class="modal-content">
              ...
              <div class="modal-footer d-flex align-items-center justify-content-between flex-wrap gap-3">
                ...
                <div class="d-flex flex-column align-items-end gap-2">
                  ...
                  <button type="button" class="btn btn-add-order" data-bs-dismiss="modal" onclick="addProductToOrder(${product.productId}, '${product.productName.replace(/'/g, "\\'")}')">Add to order</button>
                </div>
              </div>
            </div>
          </div>
...
window.updateQuantity = function (
  ...
  const cardQty = document.querySelector<HTMLElement>(`#card-qty-${productId}`);
  if (cardQty) cardQty.textContent = current.toString();

  const modalQty = document.querySelector<HTMLElement>(`#modal-qty-${productId}`);
  if (modalQty) modalQty.textContent = current.toString();

  const modalTotal = document.querySelector<HTMLElement>(`#modal-total-${productId}`);
...
window.addProductToOrder = function (productId: number, productName: string): void {
